...
export class StatsAggregator {
  async aggregate(
    ...
    const currHourStats = await timeSeriesDAL.get([...commonFields, hourTimeFrame, currHour.toMillis()])
    ...
    const last24hHourlyCandles = await timeSeriesDAL.getAllValuesFromTo(
      [...commonFields, hourTimeFrame, currHour.minus({ hours: 24 }).toMillis()],
    ...
    const last7dHourlyCandles = await timeSeriesDAL.getAllValuesFromTo(
      ...
      [...commonFields, hourTimeFrame, currHour.minus({ hours: 25 }).toMillis()],
    )
    ...
    const lastMonthDailyCandles = await timeSeriesDAL.getAllValuesFromTo(
      [...commonFields, dayTimeFrame, currDay.minus({ month: 1 }).plus({ day: 1 }).toMillis()],
    ...
    const beforeLastMonthHourlyCandles = await timeSeriesDAL.getAllValuesFromTo(
      ...
      [...commonFields, hourTimeFrame, currHour.minus({ month: 1 }).plus({ hours: 23 }).toMillis()],
    )
    ...
    const lastYearMonthlyCandles = await timeSeriesDAL.getAllValuesFromTo(
      [...commonFields, monthTimeFrame, currMonth.minus({ years: 1 }).plus({ month: 1 }).toMillis()],
      [...commonFields, monthTimeFrame, currYear.toMillis()],
    )
    const beforeLastYearDailyCandles = await timeSeriesDAL.getAllValuesFromTo(
      [...commonFields, dayTimeFrame, currDay.minus({ years: 1 }).plus({ day: 1 }).toMillis()],
      [...commonFields, dayTimeFrame, currDay.minus({ years: 1 }).plus({ month: 1 }).minus({ day: 1 }).toMillis()],
    )
    const beforeLastYearHourlyCandles = await timeSeriesDAL.getAllValuesFromTo(
      ...
      [...commonFields, hourTimeFrame, currHour.minus({ years: 1 }).plus({ hours: 23 }).toMillis()],
    )
  ...
  protected getEmptyStats(): DataFeedStats {
    return {
